...
void worker(Cell *matriz, const int fils, const int cols, const int num_threads, const int n_cpu, int *celdas_proceso, int iteraciones, int* lista_estados_procesos);
...
void *manipulatematrix(void* arg){
  ...
  for (int q = 0; q < sizeof(data -> celdas)/sizeof(int); q++){
    ...
    if ((data -> celdas)[q] + 1 < (data -> fils)*(data -> cols) && (data -> celdas)[q] + 1 >= 0){
      if ((data -> matriz_copia)[(data -> celdas)[q] + 1].cambia == 1){
        if ((data -> matriz_copia)[(data -> celdas)[q] + 1].estado == 1){
          count += 0;
        }
        else{
          count += 1;
    ...
    if (((data -> celdas)[q] + data -> cols < (data -> fils)*(data -> cols)) && ((data -> celdas)[q] + data -> cols >= 0)){
      if ((data -> matriz_copia)[(data -> celdas)[q] + data -> cols].cambia == 1){
        if ((data -> matriz_copia)[(data -> celdas)[q] + data -> cols].estado == 1){
          count += 0;
        }
        else{
          count += 1;
    ...
    if (((data -> celdas)[q] + data -> cols + 1 < (data -> fils)*(data -> cols)) && ((data -> celdas)[q] + data -> cols + 1 >= 0)){
      if ((data -> matriz_copia)[(data -> celdas)[q] + data -> cols + 1].cambia == 1){
        if ((data -> matriz_copia)[(data -> celdas)[q] + data -> cols + 1].estado == 1){
          count += 0;
        }
        else{
          count += 1;
    ...
    if ((data -> matriz_copia)[(data -> celdas)[q]].estado == 0){
      ...
      if (count == 3){
        ...
        (data -> matriz_copia)[(data -> celdas)[q]].cambia = 0;
      }
    }
...
int main(int argc, char const *argv[]) {
  ...
  file = fopen("input.txt", "r");
    ...
    int *lista_estados_procesos = mmap(0, n_cpu*sizeof(int), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);

    for (int f = 0; f < n_cpu; f++){
      lista_estados_procesos[f] = 1;
    }
    ...
    for(int i = 0; i < n_cpu; i++){
        ...
        if(j != 0){
            ...
            worker(matriz, fils, cols, num_threads, n_cpu, celdas_proceso, iteraciones, lista_estados_procesos);
          }
        }
...
void worker(Cell *matriz, const int fils, const int cols, const int num_threads, const int n_cpu, int* celdas_proceso, int iteraciones, int* lista_estados_procesos){
  ...
  while (iteraciones > 0){
    ...
    for (int f = 0; f < n_cpu; f++){
      suma_estados += lista_estados_procesos[f];
    }

    if (suma_estados == n_cpu){
      ...
      for (int x = 0; x < sizeof(celdas_proceso)/sizeof(int); x++){
        int y = celdas_proceso[x];

        if (matriz_copia[y].cambia == 1){
          matriz[y].cambia = 1;
          if (matriz[y].estado == 0){
            matriz[y].estado = 1;
          }
          else{
            matriz[y].estado = 0;
      ...
      for (int u = 0; u < n_cpu; u++){
        if (lista_estados_procesos[u] == 0){
          lista_estados_procesos[u] = 1;
        }
      }
